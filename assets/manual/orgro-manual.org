...
- Active :: <2025-11-27 Thu>
- Inactive :: [2025-11-27 Thu]
- With time :: [2025-11-27 Thu 12:34]
- Time range :: [2025-11-27 Thu 12:34-23:45]
- Date range :: [2025-11-27 Thu]--[2025-11-28 Fri]

Tap a timestamp to edit it. In the editor, long press the timestamp insertion
button to insert an active timestamp instead of an inactive one.

Active timestamps in sections indicate agenda items for which Orgro can send you
notifications; see [[*Agenda][Agenda]].
...
original file; see the [[https://orgro.org/faq/#can-i-edit-my-files-with-orgro][FAQ]] for details.
...
When you open a file containing Org Agenda items (any section with an active
timestamp like <2025-10-16 Thu>), Orgro will allow you to “add the file to your
agenda”. Look for the banner at the top of such a document.

Orgro will aggregate all the agenda items scheduled for today or later, across
all agenda files, and display them in an Agenda view. This view is available
from the start screen; look for it in the bottom navigation bar.

If you grant the required permissions, Orgro can send notifications to remind
you of your agenda items. Orgro will update pending notifications at the
following times:

- When loading or refreshing the Agenda view
- At app launch
...
You can turn notifications on and off, or clear your agenda in the Settings
screen. If you denied notification permissions, look for the “Enable
notifications” item.
...
Orgro supports transclusion of other files in the style of the [[https://nobiot.github.io/org-transclusion/][Org-transclusion]]
package. Tap the =#+transclude:= keyword to expand or collapse the referenced
content.

#+transclude: [[file:orgro-manual-demo.org][demo file]]

A subset of Org-transclusion features are supported:

- =id:= links, relative =file:= links
- =::*section-name=, =::#custom-id= targets
- =:level= property
- =:exclude-elements= and =:only-contents= properties
- =:src= with optional =:lines= property
...
Emacs. Tapping the button in the top app bar cycles between:
...
Emacs-style, but rather follow [[https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_expressions/Cheatsheet][JavaScript syntax and semantics]].
...
- ~org-log-done~ :: Automatically TODO completion time in a ~CLOSED:~ timestamp.
