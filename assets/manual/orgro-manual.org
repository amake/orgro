...
=-----BEGIN PGP MESSAGE-----=) and will offer to allow you to decrypt it. Only
symmetrically (passphrase-only) encrypted messages can be decrypted.

Note that changes to decrypted content cannot be saved.
...
Some limited editing features are available:
